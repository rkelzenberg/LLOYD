...
metadata {
        definition (name: "Settable Dimmer", namespace: "MichaelStruck", author: "SmartThings") {
        capability "Switch"
        capability "Refresh"
	...
	tiles(scale: 2) {
		multiAttributeTile(name: "switch", type: "lighting", width: 6, height: 4, canChangeIcon: true, canChangeBackground: true) {
			tileAttribute("device.switch", key: "PRIMARY_CONTROL") {
				...
				attributeState "turningOff", label: '${name}', icon: "st.switches.light.off", backgroundColor: "#ffffff", nextState: "turningOn"
		      	attributeState "turningOn", label: '${name}', icon: "st.switches.light.on", backgroundColor: "#79b821", nextState: "turningOff"
        	}
